	...
	extend (a_clause: like clause) is
		...
		require
			first_sentence_starts_with_capital_letter:
				is_empty implies not a_clause.i_th_word (1).item (1).is_alpha or else a_clause.i_th_word
(1).item (1).is_upper
		do
	...
	make (quantity: INTEGER) is
		...
		do
			...
			correct_grammatical_form_for_zero_bottles:
				quantity = 0 implies i_th_word (1).as_lower.is_equal ("no") and i_th_word (3).is_equal
(Bottle_plural)
