...
280 S=S+1:IF INT(RND(1)*2)=1 THEN PX(S)=3:PD(S)=1:PSD(S)=0ELSE PX(S)=73:\n\tPD(S)=-1:PSD(S)=1
290 PY(S)=INT(RND(1)*3)+2
300 IF S=0 THEN 280
310 REMARK \t\tDELETE OLD PLANES
320 FOR F=1 TO S:PX(F)=PX(F)+PD(F):IF PX(F)>=74 OR PX(F)<=2 THEN 370ELSE\n\tIF B=0 THEN 350ELSE FOR F1=1 TO B:IF BY(F1)<>PY(F) THEN 340
330 IF BX(F1)>=PX(F) AND BX(F1)<=PX(F)+4 THEN 380
340 NEXT F1
350 IF PY(F)<>SY THEN 360ELSE IF SX=>PX(F) AND SX<=PX(F)+4 THEN SY=0:SX=0:\n\tSB=0:GOTO 390
360 NEXT F:GOTO 410
