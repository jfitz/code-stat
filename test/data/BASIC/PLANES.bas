...
230 PRINT E$"x1"Y$"8 ";
240 PRINT "HITS: 0 bwbwbwbwbwbwbwbwbwbwbwbwbwbwb0^`^" "0bwbwbwbwbwbwbwbwbwbwbwbwbwbwbwbwbwbwb"
250 IF INT(RND(1)*15)+1<>10 THEN 300
260 IF S>2 THEN 300
270 REM  MAKE NEW PLANE
280 S=S+1:IF INT(RND(1)*2)=1 THEN PX(S)=3:PD(S)=1:PSD(S)=0ELSE PX(S)=73: PD(S)=-1:PSD(S)=1
290 PY(S)=INT(RND(1)*3)+2
300 IF S=0 THEN 280
310 REM DELETE OLD PLANES
320 FOR F=1 TO S:PX(F)=PX(F)+PD(F):IF PX(F)>=74 OR PX(F)<=2 THEN 370ELSE IF B=0 THEN 350ELSE FOR F1=1 TO B:IF BY(F1)<>PY(F) THEN 340
330 IF BX(F1)>=PX(F) AND BX(F1)<=PX(F)+4 THEN 380
340 NEXT F1
350 IF PY(F)<>SY THEN 360ELSE IF SX=>PX(F) AND SX<=PX(F)+4 THEN SY=0:SX=0: SB=0:GOTO 390
360 NEXT F:GOTO 410
370 PRINT Y$CHR$(PY(F)+31)CHR$(PX(F)+31)P$(2);:S=S-1:IF S=0 THEN 420: ELSE FOR Z=F TO S:PX(Z)=PX(Z+1):PY(Z)=PY(Z+1):PD(Z)=PD(Z+1): PSD(Z)=PSD(Z+1):NEXT Z:GOTO 360
380 B=B-1:FOR F2=F1 TO B:BX(F2)=BX(F2+1):BD(F2)=BD(F2+1):BY(F2)=BY(F2+1):NEXT F2
390 PRINT Y$CHR$(PY(F)+31)CHR$(PX(F)+31)"\a*BOOM*"Y$CHR$(PY(F)+31) CHR$(PX(F)+31);"       ";:H0=H0+1:PRINT Y$;"8 HITS:";H0;H$;:GOTO 370
400 REM PRINT SHIPS
...
450 BY(F)=BY(F)-1:BX(F)=BX(F)+BDX(F): IF BX(F)<=0 OR BY(F)<=0 OR BX(F)>=80 THEN 460: ELSE PRINT Y$;CHR$(BY(F)+31);CHR$(BX(F)+31);"^";:NEXT F:GOTO 480
...
490 PRINT Y$CHR$(SY+31)CHR$(SX+31)" ";:SX=SX+SD:SY=SY-1: IF SX<=0 OR SY<=0 OR SX>=80 THEN SX=0:SY=0:SB=0: ELSE PRINT Y$;CHR$(SY+31);CHR$(SX+31);"*";
500 REM
...
560 GOTO 300
570 IF BDX<=-2 THEN 630ELSE BDX=BDX-1:GOTO 630
580 IF B>2 THEN 250ELSE B=B+1:BX(B)=40:BY(B)=23:BDX(B)=BDX:GOTO 630
590 IF BDX>=2 THEN 250ELSE BDX=BDX+1:GOTO 630
600 IF SD<=-4 THEN 640ELSE SD=SD-1:GOTO 640
610 IF SB>0 THEN 640ELSE SB=1:SX=40:SY=23:GOTO 640
620 IF SD>=4 THEN 640ELSE SD=SD+1:GOTO 640
